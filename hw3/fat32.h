...
typedef struct struct_BPBFAT32_struct {
    ...
    uint16_t BkBootSec;     // First logical sector number of a copy of the three FAT32 boot sectors. It is typically 6.
                            // Size: 2 bytes
    ...
    uint8_t BS_FileSystemType[8]; // File system type. Padded with spaces at the end. In our case it will be "FAT32   ".
                                  // Size: 8 bytes
} BPB32_struct;
...
typedef struct struct_FSInfo {
    uint32_t signature;    // Signature. 0x41615252 for FAT32.
    uint8_t reserved[480]; // Reserved.
    uint32_t signature2;   // Signature. 0x61417272 for FAT32.
    uint32_t freeClusters; // Number of free clusters.
    uint32_t nextFree;     // First cluster of the free cluster list.
    uint8_t reserved2[12]; // Reserved.
    uint32_t signature3;   // Signature. 0xAA550000 for FAT32.
} FSInfo;
